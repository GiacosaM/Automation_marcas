...
class EmailsPage:
    ...
    def _ensure_connection(self, conn):
        """Garantiza que la conexión esté abierta; si no, crea una nueva."""
        try:
            if conn is None:
                return crear_conexion()
            # psycopg2 tiene atributo closed (0 abierto, 1 cerrado). sqlite3 no.
            if hasattr(conn, 'closed') and conn.closed:
                return crear_conexion()
            # Prueba rápida (evita usar si está abortada)
            try:
                cur = conn.cursor()
                ...
                with st.expander("👀 Previsualizar Envíos", expanded=False):
                    try:
                        ...
                        if usar_supabase_simple():
                            ...
                            query_preview = convertir_query_boolean("""
                                SELECT b.titular, c.email, COUNT(*) AS cantidad_reportes,
    ...
    def _show_historial_envios_tab(self, conn):
        ...
        # Asegurar conexión (abrir una nueva para este bloque si la pasada está cerrada)
        try:
            try:
            ...
            cursor = conn.cursor()
            query_historial = convertir_query_boolean("""
            ...
            if historial_envios:
                ...
                with col1:
                    tipo_filter = st.multiselect(
                        "Filtrar por tipo:",
                        options=["informes", "notificacion"],
                        default=["informes", "notificacion"],
                        format_func=lambda x: "Informes" if x == "informes" else "Notificaciones"
                    )
                with col2:
                    titular_filter = st.text_input("Filtrar por titular:", placeholder="Ingrese nombre del titular...")
                
                # Aplicar filtros
                filtered_df = historial_df
                
                if tipo_filter and len(tipo_filter) < 2:
                    filtered_df = filtered_df[filtered_df['Tipo'].isin(tipo_filter)]
                
                if titular_filter:
                    filtered_df = filtered_df[filtered_df['Titular'].str.contains(titular_filter, case=False)]
    ...
    def show(self):
        ...
        if conn:
            try:
                # Crear tabla de emails y de boletines antes de usar el sistema
                crear_tabla(conn)
                cursor = conn.cursor()
                from db_utils import usar_supabase_simple
                if usar_supabase_simple():
                ...
                # Verificamos si la columna titular ya existe (solo para SQLite legacy)
                try:
                    from database import usar_supabase, obtener_columnas_tabla
                    
                    if not usar_supabase():
                        # Solo para SQLite - verificar y agregar columnas
                    ...
                    # Para PostgreSQL, las tablas ya tienen la estructura completa
                        
                except ImportError:
                    # Fallback si no se puede importar las nuevas funciones
                    pass
                
                cursor.close()
                
                # Obtener estadísticas específicas del sistema de envío (después de asegurar tablas)
