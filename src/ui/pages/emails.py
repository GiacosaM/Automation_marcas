...
from database import crear_conexion, crear_tabla, obtener_logs_envios, obtener_estadisticas_logs, limpiar_logs_antiguos, obtener_emails_enviados
...
class EmailsPage:
    ...
    def _show_email_analytics(self, stats):
        ...
        if stats['por_dia']:
            with st.expander("📈 Actividad Semanal", expanded=False):
                import plotly.graph_objects as go
                
                dias = [item[0] for item in stats['por_dia']]
                cantidades = [item[1] for item in stats['por_dia']]
                ...
                fig.update_layout(
                    title="Emails Enviados por Día (Última Semana)",
                    xaxis_title="Fecha",
                    yaxis_title="Cantidad de Emails",
                    height=400,
                    showlegend=False
                )
    ...
    def _show_email_management_system(self, conn, stats):
        ...
        tab1, tab2, tab3, tab4, tab5 = st.tabs([
            "🚀 Enviar Emails", "⚙️ Configuración", "📊 Historial de Envíos", 
            "📋 Logs Detallados", "📁 Emails Enviados"
        ])
        ...
        with tab2:
            self._show_configuracion_tab()
        
        with tab3:
            self._show_historial_envios_tab(conn)
        
        with tab4:
            self._show_logs_detallados_tab(conn)
        
        with tab5:
            self._show_emails_enviados_tab(conn)
    ...
    def _show_historial_envios_tab(self, conn):
        ...
        st.info("📝 Funcionalidad de historial de envíos - Por implementar")
    
    def _show_logs_detallados_tab(self, conn):
        """Mostrar tab de logs detallados"""
        st.markdown("### 📋 Logs Detallados de Envíos")
        st.info("📝 Funcionalidad de logs detallados - Por implementar")
    
    def _show_emails_enviados_tab(self, conn):
        """Mostrar tab de emails enviados"""
        st.markdown("### 📁 Historial de Emails Enviados")
        st.info("📝 Funcionalidad de emails enviados - Por implementar")
    ...
    def show(self):
        ...
        if conn:
            try:
                ...
                # Analíticas
                self._show_email_analytics(stats_simple)
                
            except Exception as e:
                st.error(f"Error: {e}")
                # Fallback al sistema simple si hay errores
                stats = self._get_email_stats(conn)
