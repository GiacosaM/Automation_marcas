#!/usr/bin/env python3
"""
Script de migración final de SQLite a Supabase con conversiones de tipos
"""
...
def crear_tablas_supabase():
    """Crear tablas en Supabase con estructura SQLite original y tipos corregidos"""
    ...
    try:
        ...
        # Crear tabla boletines - tipos corregidos
        cursor.execute("""
        ...
        # Crear tabla clientes - tipos corregidos (CUIT como BIGINT)
        cursor.execute("""
...
def convertir_valor(valor, tipo_campo):
    ...
    # Conversión de booleanos (SQLite usa 0/1)
    if tipo_campo == 'boolean':
        return bool(valor)
    
    # Conversión de enteros largos (CUIT)
    if tipo_campo == 'bigint':
        try:
...
def importar_datos_a_supabase(datos):
    """Importar datos a Supabase con conversiones de tipos"""
    print("\n📥 Importando datos a Supabase...")
    
    # Definir tipos de campos que necesitan conversión
    tipos_campos = {
        'boletines': {
    ...
    try:
        ...
        for tabla, registros in datos.items():
            ...
            # Insertar registros uno por uno con conversiones
            exitosos = 0
            for i, registro in enumerate(registros):
                try:
                    ...
                    for col in columnas:
                        valor = registro[col]
                        tipo_campo = tipos_campos.get(tabla, {}).get(col, 'text')
                        valor_convertido = convertir_valor(valor, tipo_campo)
            ...
            print(f"✅ {tabla}: {exitosos} registros procesados exitosamente")
...
def main():
    ...
    print("🚀 Iniciando migración final de SQLite a Supabase")
    print("=" * 50)
